...
public class FollowMouseByForce extends Function{
	...
	// Value - Mode
	boolean loveMouse = true;
	
	// Constructor
	public FollowMouseByForce(WorldComponent master) {super(master);}
	...
	public void additionalUpdate() {
		...
		if(playerCenter != null && activate()) {
			...
			int speed = 0;
			if(loveMouse) {speed = this.speed * (int)playerCenter.distance(masterCenter)/distanceSpeedFactor;}
			else{speed = - this.speed / (int)playerCenter.distance(masterCenter)*distanceSpeedFactor;}
	...
	public void setLoveMouse(boolean boo) {this.loveMouse = boo;}
